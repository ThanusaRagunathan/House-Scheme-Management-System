  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const combined = [
          ...payments.map(p => ({ 
            name: p.TenantName || 'Tenant Payment', 
            amount: `+ Rs. ${parseFloat(p.amount).toLocaleString()}`, 
            status: 'Success', 
            date: new Date(p.paid_date).toLocaleDateString(), 
            isIncome: true 
          })),
          ...maintenances.map(m => ({ 
            name: m.description, 
            amount: `- Rs. ${parseFloat(m.cost).toLocaleString()}`, 
            status: 'Success', 
  ...
  return (
    ...
    >
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(280px, 1fr))", gap: "25px", marginBottom: "40px" }}>
        ...
        <StatCard title="Active Tenancies" value={loading ? "..." : stats.activeTenancies} subtitle="Managed Tenants" icon="bi-house-heart" color="#9b59b6" />
      </div>
